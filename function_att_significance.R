...
att.significance <- function(match.obj=match.obj, grouping_var = NULL, conf = NULL, sig.test = NULL, p.value = NULL) {
...
# Dealing with  conf level, pval, etc (defining a default if they were not defined in function call)
  ...
  reg.data$MU.X <- predict(reg.fit,reg.data,weights=w)  # predict Y based on ctrl model
