...
# 6) ecaliper: caliper that factors in exact matches, in sd units
  ...
  # not sure what is the difference between cols 1 and 4, 2 and 5.
   ...
   if (indx[1, 1] == 0) {  # if no matches returned?
    ret <- list()
    ret$valid <- 0
    if (caliperflag) {
  ...
  if (All == 2) {    # only for ATC, switch around Tr and Ctrls
    foo <- indx[, 5]
    indx[, 5] <- indx[, 4]
    indx[, 4] <- foo
  }
  if (caliperflag) {
    sum.caliper.drops <- indx[1, 6]   # perhaps col 6 contains caliper drops (missing if none)
  }
  else {
    sum.caliper.drops <- 0
  }
  I <- indx[, 1] # matched index
  IT <- Tr[indx[, 1]] # matched treated values (generally all 1s)
  IM <- indx[, 2] # index for matched treated
  ...
  if (BiasAdj == 1) {
    ...
    IZ <- Z[indx[, 1], ] # matched index (for bias adj)
  ...
  est.func <- function(N, All, Tr, indx, weight, BiasAdj, 
                       Kz) {
    Kcount <- as.matrix(rep(0, N))   # empty matrix for 
    KKcount <- as.matrix(rep(0, N))  # empty matrix (vector) for 
    ...
    for (i in 1:N) {
      if ((Tr[i] == 1 & All != 1) | All == 1) {  # for each treated value
        foo.indx <- indx[, 1] == i # indicates if obs is matched treated obs.
        foo.indx2 <- foo.indx # index for  (treated) obs i (multiple when multiple matches)
        sum.foo <- sum(foo.indx) # number of matches for obs i 
        if (sum.foo < 1) # not a matched obs
          next   # stop loop and skip to the next one (i.e. ignores obs not incl in match)
        foo <- rep(FALSE, N)
        foo.indx <- indx[foo.indx, 2] # index for ctrl matched to obs i 
        foo[foo.indx] <- rep(TRUE, sum.foo) # foo TRUE for matched ctrl obs  
        Kcount <- Kcount + weight[i] * weight * foo/sum(foo * 
                                                          weight) # weights for ctrl matches (for each i) accounting for all weighting
        KKcount <- KKcount + weight[i] * weight * weight * 
          foo/(sum(foo * weight) * sum(foo * weight)) # weights^2 for ctrl matches (for each i) accounting for all weighting
        foo.indx2.2 <- indx[foo.indx2, 2] # index for ctrl matched to obs i (same as foo.indx?? line 178)
        foo.indx2.3 <- indx[foo.indx2, 3] # weights for ctrl matched to obs i 
        sum.foo.indx2.3 <- sum(foo.indx2.3) # sum of weights (always 1???)
        if (Tr[i] == 1) {   # if treated
          YCAUS[i] <- Y[i] - sum((Y[foo.indx2.2] * foo.indx2.3))/sum.foo.indx2.3  # Y(tr) less avg Y of matches
        }
        else { # if ctrl site
          YCAUS[i] <- sum((Y[foo.indx2.2] * foo.indx2.3))/sum.foo.indx2.3 -       # avg Y of matches less Y(tr)
            Y[i]
        }
        if (BiasAdj == 1) {
          if (Tr[i] == 1) {
            if (sum.foo > 1) {  # if >1 matches
              ZCAUS[i, ] <- Z[i, ] - t(Z[foo.indx2.2, 
                                         ]) %*% foo.indx2.3/sum.foo.indx2.3      # avg Z(tr) less avg Z values for matches)
            }
            else { # if 1 match
  ...
  if (All != 1) {
    ...
    I <- as.matrix(I[IT == 1])  # matched index where Tr=1 . 
    IT <- as.matrix(IT[IT == 1])  # matched treated index where Tr=1 
    ...
    if (BiasAdj == 1) {
      if (Kz > 1) { #if multiple covariates for bias adjustment
  ...
  if (BiasAdj == 1) {
    ...
    if (All == 1) { 
      ...
      xw <- ZZt * (sqrt(Tr * Kcount) %*% t(as.matrix(rep(1, 
                                                         Kx)))) # changes the Kcount from vector to a nrow(Kcount x Kx) matrix (columns identical)
  ...
  if (BiasAdj == 1) {
    ...
    xw <- ZZc * (sqrt((1 - Tr) * Kcount) %*% matrix(1, nrow = 1, 
                                                    ncol = Kx)) # changes the Kcount from vector to a nrow(Kcount x Kx) matrix (columns identical)
    foo <- min(eigen(t(xw) %*% xw, only.values = TRUE)$values)
    foo <- as.double(foo <= ccc)
    foo2 <- apply(xw, 2, sd)
    options(show.error.messages = FALSE)
  ...
  if (BiasAdj == 1) {
    ...
    SCAUS <- YCAUS - Tr * (ZCAUS %*% Alphac) - (1 - Tr) * 
      (ZCAUS %*% Alphat)
  ...
  if (version == "standard") {
    if (Var.calc == 0) {
      ...
      eps.sq <- eps * eps
      Sigs <- 0.5 * matrix(1, N, 1) %*% (t(eps.sq) %*% 
                                           W)/sum(W)
    }
    SN <- sum(iot.t)
    var.sample <- sum((Sigs * (iot.t + iot.c * Kcount) * 
                         (iot.t + iot.c * Kcount))/(SN * SN))
    if (All == 1) {
